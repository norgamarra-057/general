...
import (
	...
	"config-updater/internal/telegraf"
)
...
func (kube *Kube) CreateInputConfigMap(data map[string]string) *v1.ConfigMap {
	...
	cm, err := kube.clientset.CoreV1().ConfigMaps(kube.Ns).Create(&v1.ConfigMap{
	...
	zap.L().Info("CreateInputConfigMap", zap.String("name", cm.ObjectMeta.Name))
	return cm
}

func (kube *Kube) ParseYamlTelegrafDeployment() (deployment *appsv1.Deployment) {
	f, err := os.Open("/etc/config-updater/telegraf-raas-deployment.yml")
	if err != nil {
		panic(err.Error())
	}
	defer f.Close()
...
func (kube *Kube) SetTelegrafConfigMapName(d *appsv1.Deployment, name string) {
	// fill in the configMap's name. See telegraf-raas-deployment.yml:
	// volumes:
...
func (kube *Kube) CreateInputConfigMapData(servers []raas.Server) map[string]string {
	data := map[string]string{
		"redis_input.conf":             telegraf.RedisInputConf(servers),
		"redis_latency_input.conf":     telegraf.RedisLatencyInputConf(servers),
		"memcached_input.conf":         telegraf.MemcachedInputConf(servers),
		"memcached_latency_input.conf": telegraf.MemcachedLatencyInputConf(servers),
	}
	return data
}

func (kube *Kube) NewTelegrafDeployment(servers []raas.Server) *appsv1.Deployment {
...
func (kube *Kube) DeployTelegraf(servers []raas.Server) {
	...
	zap.L().Info("deployment", zap.String("method", "Update"))
	result, err := deploymentsClient.Update(deployment)
	if errors.IsNotFound(err) {
		zap.L().Info("deployment", zap.String("method", "Create"))
		result, err = deploymentsClient.Create(deployment)
	}
	if err != nil {
		panic(err)
	}
	zap.L().Info("deployment", zap.String("name", result.GetObjectMeta().GetName()))
}
