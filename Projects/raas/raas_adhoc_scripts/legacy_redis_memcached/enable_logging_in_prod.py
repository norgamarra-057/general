...
if [ -n "$port_no" ]; then 
    if  [[ `sudo iptables -L INPUT --line-numbers --numeric |  grep ":${port_no}" | grep "LOG" | wc -l ` -eq 0 ]]; then 
        # echo "DummyIptablesCommand"
        sudo iptables -I INPUT -p tcp -m tcp --dport ${port_no} -m state --state NEW  -j LOG --log-level 1 --log-prefix "${port_no}-new-connection:"
        if [[ $? -eq 0 ]]; then 
            echo "iptables ${port_no}: log rule applied successfully"
        else
            echo "iptables ${port_no}: log rule apply failed"
        fi
    else
            echo "iptables ${port_no}: log rule already exists"
    fi
