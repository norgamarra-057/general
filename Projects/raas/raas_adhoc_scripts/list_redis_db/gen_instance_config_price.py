...
req_mem=200
region_code='eu-west-1' # us-west-1, us-west-2, eu-west-1
cache_engine='Redis'    # Redis, Memcached
db_type='standalone'    # standalone, cluster
data_tier=False
...
SELECT a.*, (cast(b.seq_no as varchar(5))||' x '||a.instanceType) InstanceConfig , {mem_formula1} AllocatedMemory, {mem_formula1}+ssd TotalMemoryWithSSD , b.seq_no*a.pricePerUnit*24*30 as "Price_No_rep($/Month)", 2*b.seq_no*a.pricePerUnit*24*30 as "Price_With_rep($/Month)"
...
df[['InstanceConfig','Price_With_rep($/Month)']].to_csv('euw1_price.csv',index=False)
...
df[['InstanceConfig','Price_With_rep($/Month)']].to_csv('usw1_price.csv',index=False)