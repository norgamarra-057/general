...
func (m *Mysql) setupLogger() {
	...
	m.logger = zap.New(core, zap.AddStacktrace(zap.ErrorLevel))
}
...
func (m *Mysql) connectDBs() {
	m.logger.Info("connectDBs", zap.Int("servers", len(m.Servers)))
	m.dbs = make(map[string]*sql.DB)
	for _, serv := range m.Servers {
		m.logger.Info("connecting", zap.String("server", m.sanitizedAddress(serv)))
		err := m.connectDB(serv)
		if err != nil {
			m.logger.Error("error connecting", zap.String("server", m.sanitizedAddress(serv)), zap.Error(err))
		}
	}
}
...
func (m *Mysql) getDB(serv string) (*sql.DB, error) {
	if m.dbs[serv] == nil {
		m.logger.Info("reconnecting", zap.String("server", m.sanitizedAddress(serv)))
		err := m.connectDB(serv)
		if err != nil {
			return nil, err
		}
...
func (m *Mysql) gatherServer(serv string, acc telegraf.Accumulator) error {
	db, err := m.getDB(serv)
	if err != nil {
		m.logger.Error("getDB",
			zap.String("server", m.sanitizedAddress(serv)),
			zap.Error(err))
		err = fmt.Errorf("%v (%v)", err, m.sanitizedAddress(serv))
		return err
	}

	err = m.gatherGlobalStatuses(db, serv, acc)
	if err != nil {
		m.logger.Error("gatherGlobalStatuses",
			zap.String("server", m.sanitizedAddress(serv)),
			zap.Error(err))
		err = fmt.Errorf("%v (%v)", err, m.sanitizedAddress(serv))
		return err
	...
	if m.GatherGlobalVars {
		...
		if len(m.IntervalSlow) > 0 {
			if uint32(time.Since(m.lastT).Seconds()) >= m.scanIntervalSlow {
				...
				if err != nil {
					m.logger.Error("gatherGlobalVariables",
						zap.String("server", m.sanitizedAddress(serv)),
						zap.Error(err))
					err = fmt.Errorf("%v (%v)", err, m.sanitizedAddress(serv))
					return err
	...
	if m.GatherBinaryLogs {
		...
		if err != nil {
			m.logger.Error("gatherBinaryLogs",
				zap.String("server", m.sanitizedAddress(serv)),
				zap.Error(err))
			err = fmt.Errorf("%v (%v)", err, m.sanitizedAddress(serv))
			return err
	...
	if m.GatherProcessList {
		...
		if err != nil {
			m.logger.Error("GatherProcessListStatuses",
				zap.String("server", m.sanitizedAddress(serv)),
				zap.Error(err))
			err = fmt.Errorf("%v (%v)", err, m.sanitizedAddress(serv))
			return err
	...
	if m.GatherUserStatistics {
		...
		if err != nil {
			m.logger.Error("GatherUserStatisticsStatuses",
				zap.String("server", m.sanitizedAddress(serv)),
				zap.Error(err))
			err = fmt.Errorf("%v (%v)", err, m.sanitizedAddress(serv))
			return err
	...
	if m.GatherSlaveStatus {
		...
		if err != nil {
			m.logger.Error("gatherSlaveStatuses",
				zap.String("server", m.sanitizedAddress(serv)),
				zap.Error(err))
			err = fmt.Errorf("%v (%v)", err, m.sanitizedAddress(serv))
			return err
	...
	if m.GatherInfoSchemaAutoInc {
		...
		if err != nil {
			m.logger.Error("gatherInfoSchemaAutoIncStatuses",
				zap.String("server", m.sanitizedAddress(serv)),
				zap.Error(err))
			err = fmt.Errorf("%v (%v)", err, m.sanitizedAddress(serv))
			return err
	...
	if m.GatherInnoDBMetrics {
		...
		if err != nil {
			m.logger.Error("gatherInnoDBMetrics",
				zap.String("server", m.sanitizedAddress(serv)),
				zap.Error(err))
			err = fmt.Errorf("%v (%v)", err, m.sanitizedAddress(serv))
			return err
	...
	if m.GatherPerfSummaryPerAccountPerEvent {
		...
		if err != nil {
			m.logger.Error("gatherPerfSummaryPerAccountPerEvent",
				zap.String("server", m.sanitizedAddress(serv)),
				zap.Error(err))
			err = fmt.Errorf("%v (%v)", err, m.sanitizedAddress(serv))
			return err
	...
	if m.GatherTableIOWaits {
		...
		if err != nil {
			m.logger.Error("gatherPerfTableIOWaits",
				zap.String("server", m.sanitizedAddress(serv)),
				zap.Error(err))
			err = fmt.Errorf("%v (%v)", err, m.sanitizedAddress(serv))
			return err
	...
	if m.GatherIndexIOWaits {
		...
		if err != nil {
			m.logger.Error("gatherPerfIndexIOWaits",
				zap.String("server", m.sanitizedAddress(serv)),
				zap.Error(err))
			err = fmt.Errorf("%v (%v)", err, m.sanitizedAddress(serv))
			return err
	...
	if m.GatherTableLockWaits {
		...
		if err != nil {
			m.logger.Error("gatherPerfTableLockWaits",
				zap.String("server", m.sanitizedAddress(serv)),
				zap.Error(err))
			err = fmt.Errorf("%v (%v)", err, m.sanitizedAddress(serv))
			return err
	...
	if m.GatherEventWaits {
		...
		if err != nil {
			m.logger.Error("gatherPerfEventWaits",
				zap.String("server", m.sanitizedAddress(serv)),
				zap.Error(err))
			err = fmt.Errorf("%v (%v)", err, m.sanitizedAddress(serv))
			return err
	...
	if m.GatherFileEventsStats {
		...
		if err != nil {
			m.logger.Error("gatherPerfFileEventsStatuses",
				zap.String("server", m.sanitizedAddress(serv)),
				zap.Error(err))
			err = fmt.Errorf("%v (%v)", err, m.sanitizedAddress(serv))
			return err
	...
	if m.GatherPerfEventsStatements {
		...
		if err != nil {
			m.logger.Error("gatherPerfEventsStatements",
				zap.String("server", m.sanitizedAddress(serv)),
				zap.Error(err))
			err = fmt.Errorf("%v (%v)", err, m.sanitizedAddress(serv))
			return err
	...
	if m.GatherTableSchema {
		...
		if err != nil {
			m.logger.Error("gatherTableSchema",
				zap.String("server", m.sanitizedAddress(serv)),
				zap.Error(err))
			err = fmt.Errorf("%v (%v)", err, m.sanitizedAddress(serv))
			return err
...
func (m *Mysql) gatherGlobalVariables(db *sql.DB, serv string, acc telegraf.Accumulator) error {
	...
	for rows.Next() {
		...
		if err != nil {
			...
			m.logger.Error("error parsing mysql global variable",
				zap.String("server", m.sanitizedAddress(serv)),
				zap.String("key", key),
	...
	for rows.Next() {
		...
		for i, col := range cols {
			...
			if err != nil {
				...
				m.logger.Error("error parsing mysql slave status",
					zap.String("server", m.sanitizedAddress(serv)),
					zap.String("colName", colName),
					zap.String("colValue", string(colValue)),
					zap.Error(err))
				if m.MetricVersion >= 2 {
					acc.AddError(errString)
				}
...
func (m *Mysql) gatherInnoDBMetrics(db *sql.DB, serv string, acc telegraf.Accumulator) error {
	...
	for rows.Next() {
		...
		if err != nil {
			m.logger.Error("error parsing mysql InnoDB metric",
				zap.String("server", m.sanitizedAddress(serv)),
				zap.String("key", key),
				zap.String("val", string(val)),
				zap.Error(err))
...
func (m *Mysql) parseValueByDatabaseTypeName(value sql.RawBytes, databaseTypeName string) (interface{}, error) {
	...
	default:
		m.logger.Error("unknown database type", zap.String("databaseTypeName", databaseTypeName))
