# Sean's .cshrc file. Why? 'cause you can't take the suck out of bash(1) no
# matter how hard you try. Use POSIX sh(1) for scripts and zsh(1) or tcsh(1)
# for your day-to-day shell. Usable zsh(1) is a little too heavyweight to
# setup everywhere, and tcsh(1) gets the job done quickly/easily.
#
...
foreach d (/usr/local/sbin /opt/local/sbin /usr/local/bin /opt/local/bin $HOME/sbin $HOME/bin ${GOPATH}/bin /usr/bin /usr/sbin /bin /sbin $path)
	if ( -d $d ) then
...
foreach f (/etc/ssh/known_hosts /etc/ssh_known_hosts $HOME/.ssh/known_hosts )
	if ( -r $f && ! -z $f ) then
		...
		set ssh_hosts=`grep '^\[' ${HOME}/.ssh/known_hosts | cut -d '[' -f 2 | cut -d ']' -f 1` >& /dev/null
		set -f hosts=($hosts $ssh_hosts)

		set ssh_hosts=`grep -v '^\[' ${HOME}/.ssh/known_hosts | sed 's/[, ].*//'` >& /dev/null
		set -f hosts=($hosts $ssh_hosts)
		unset ssh_hosts

		# Now snag random ssh ports
		set tmp_ports=`grep '^\[' ${HOME}/.ssh/known_hosts | awk -F : '{print $2}' | awk '{print $1}'` >& /dev/null
...
if ( ! -o /tmp/tmp.${USER} || ! -P77: /tmp/tmp.${USER} == "0" ) then
  # Clear out untrusted items
  /bin/rm ~/tmp
  /bin/rm -rf /tmp/tmp.${USER}
endif
