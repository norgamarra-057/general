...
function ss_get_mysql_stats( $options ) {
   ...
   if ( $chk_options['slave'] ) {
      ...
      foreach ( $result as $row ) {
         ...
         # check pt-heartbeat lag
         if ( $status['read_only'] == 'OFF' ) {
            $status['slave_time_behind_master'] = 0;
         } else {
            $lag_seconds = exec("/usr/local/bin/pt-heartbeat --check -D percona --master-server-id $master_server_id", $pt_out, $pt_rv);
            if ($pt_rv > 0) {
               # we want a failed execution to set a sufficiently large lag value
               # so it would likely trigger an alert for the operator to take action.
               # possible actions would be: install percona tools, start pt-heartbeat
               # daemon on the master, or ACK the alert.
               $status['slave_time_behind_master'] = 9999;
            } else {
               $status['slave_time_behind_master'] = $lag_seconds;
            }
         }
