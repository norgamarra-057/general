...
function find_connections {
    Vip="$1"
    Dbname="$2"
    result=$(psql -p 5432 -h $Vip -d $Dbname -t -x -U gds_admin -c "select usename,client_addr,count(*) from pg_stat_activity where datname='$Dbname' group by 1,2 order by 3 desc")
    echo "****************************************************Connections:"
    echo "${result}"
    echo "--------------------------------------------------------------------------------------------------------------------------------"
}
