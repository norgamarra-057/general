...
db_list=get_redis_db_list('us-west-1')
...
df.to_csv('redis_db_list_usw1.csv',index=True)

'''
sqlite3 <<EOT
.mode ascii
.separator "," "\n"
.import redis_db_list_usw1.csv redis_db

.header on 

select name, count(*) no_of_nodes,max(port) port, max(node_type) node_type, max(db_type) db_type, max(redis_version) redis_version, max(uptime_in_days) uptime_in_days, sum(used_memory) used_memory, sum(used_memory_peak) used_memory_peak, sum(maxmemory) maxmemory from redis_db group by 1;
